/*
  # Popular banco de dados com dados completos

  1. Dados inseridos
    - 30 colaboradores completos do arquivo CSV
    - Configurações de peso padrão
    - Alertas automáticos baseados nos dados
    - Avaliações históricas
    - Orçamentos por centro de custos
    - Tabela salarial de referência

  2. Funcionalidades
    - Cálculo automático de scores
    - Geração de alertas inteligentes
    - Análise de sentimento simulada
    - Detecção de anomalias de absenteísmo
*/
...
RETURNS decimal AS $$
BEGIN
  RETURN ROUND(EXTRACT(EPOCH FROM (CURRENT_DATE - admission_date)) / (365.25 * 24 * 60 * 60), 1);
END;
$$ LANGUAGE plpgsql;
