/*
  ...
  1. Configurações
    - Configuração padrão de pesos para cálculo de scores
    
  2. Dados dos colaboradores
    - 30 colaboradores do CSV original
    - Dados completos com cálculos automáticos
    
  3. Alertas automáticos
    - Alertas baseados em risco, performance e absenteísmo
    - Alertas departamentais
    
  4. Avaliações e histórico
    - Avaliações históricas para todos os colaboradores
    - Exemplos de análises no histórico
*/
...
-- Atualizar campos calculados para todos os colaboradores usando cálculos diretos
UPDATE employees SET
  -- Calcular tempo de casa em anos usando diferença de datas
  tempo_de_casa = ROUND((CURRENT_DATE - data_admissao)::numeric / 365.25, 1),
  -- Calcular tempo no cargo em anos
  tempo_no_cargo = ROUND(tempo_cargo_atual_meses::decimal / 12, 1),
  -- Calcular absenteísmo como percentual
  absenteismo = ROUND(faltas_injustificadas::decimal / 100, 3);

-- Calcular score de experiência
UPDATE employees SET
  score_experiencia = ROUND(
    (LEAST((CURRENT_DATE - data_admissao)::numeric / (365.25 * 10), 1) * 0.7 + 
     LEAST(tempo_cargo_atual_meses::decimal / (12 * 5), 1) * 0.3) * 100, 1
  );

-- Calcular score de diversidade
UPDATE employees SET
  score_diversidade = LEAST(
    (CASE WHEN genero != 'Masculino' THEN 20 ELSE 0 END) +
    (CASE WHEN raca_cor != 'Branca' THEN 20 ELSE 0 END) +
    (CASE WHEN orientacao_sexual != 'Heterosexual' THEN 20 ELSE 0 END) +
    (CASE WHEN grau_escolaridade ILIKE '%Superior%' OR grau_escolaridade ILIKE '%Pós%' THEN 20 ELSE 0 END) +
    (CASE WHEN idiomas_falados IS NOT NULL AND idiomas_falados != 'Português' THEN 20 ELSE 0 END),
    100
  );

-- Calcular score de formação
UPDATE employees SET
  score_formacao = LEAST(
    (CASE 
      WHEN grau_escolaridade ILIKE '%Pós%' OR grau_escolaridade ILIKE '%Mestrado%' OR grau_escolaridade ILIKE '%Doutorado%' THEN 40
      WHEN grau_escolaridade ILIKE '%Superior Completo%' THEN 30
      WHEN grau_escolaridade ILIKE '%Superior%' THEN 20
      WHEN grau_escolaridade ILIKE '%Médio%' THEN 10
      ELSE 5
    END) +
    (CASE WHEN cursos_concluidos IS NOT NULL AND cursos_concluidos != 'Nenhuma' AND cursos_concluidos != '' THEN 20 ELSE 0 END) +
    (CASE WHEN certificacoes_relevantes IS NOT NULL AND certificacoes_relevantes != 'Nenhuma' AND certificacoes_relevantes != '' THEN 30 ELSE 0 END),
    100
  );

-- Gerar feedback simulado baseado no desempenho
UPDATE employees SET
...
-- Calcular score de sentimento baseado no feedback
UPDATE employees SET
  score_sentimento = CASE 
    WHEN feedback_ultima_avaliacao ILIKE '%precisa%' OR feedback_ultima_avaliacao ILIKE '%necessita%' THEN 0.7
    WHEN feedback_ultima_avaliacao ILIKE '%potencial%' OR feedback_ultima_avaliacao ILIKE '%desenvolvimento%' THEN 0.4
    WHEN feedback_ultima_avaliacao ILIKE '%excepcional%' OR feedback_ultima_avaliacao ILIKE '%referência%' THEN 0.1
    WHEN feedback_ultima_avaliacao ILIKE '%sólido%' OR feedback_ultima_avaliacao ILIKE '%confiável%' THEN 0.2
    ELSE 0.5
  END;

-- Marcar absenteísmo anômalo
UPDATE employees SET
...
UPDATE employees SET
  risk_factors = ARRAY_REMOVE(ARRAY[
    ...
    CASE WHEN (CURRENT_DATE - data_ultima_promocao)::numeric / 365.25 > 3 THEN 'Sem promoção há mais de 3 anos' ELSE NULL END,
