/*
  # Popular banco de dados com colaboradores completos
  ...
  2. Dados dos Colaboradores
    - 30 colaboradores dos departamentos Comercial e Administrativo
    - Dados completos incluindo salários, avaliações, formação

  3. Cálculos Automáticos
    - Tempo de casa e no cargo
    - Scores de experiência, diversidade e formação
    - Score final baseado em fórmula ponderada
...
BEGIN
  RETURN ROUND(
    EXTRACT(EPOCH FROM (CURRENT_DATE - admission_date)::interval) / (365.25 * 24 * 60 * 60), 
    1
  );
END;
$$ LANGUAGE plpgsql;
